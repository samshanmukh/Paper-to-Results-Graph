...
The **Ask the research agent** panel routes to a RocketRide pipeline (`pipelines/paper2result.pipe`) running on the local engine. The agent works in *waves*: it reads your question, decides what to query, has the `db_neo4j` tool translate natural language into Cypher, inspects the results, queries again if needed, and then synthesizes an answer that cites claim ids, run ids, and **exact metrics** (an accuracy rule in its instructions forbids rounding or estimating).
...
## 8. Sponsor stack — what each partner does
...
## 9. Repo map — every file explained
...
## 12. Status — everything verified end-to-end
